...
class MarineWizEngineeringSupabaseUser extends BaseAuthUser {
  MarineWizEngineeringSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> marineWizEngineeringSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MarineWizEngineeringSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
